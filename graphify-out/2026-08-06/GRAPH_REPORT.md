# Graph Report - Pro-Dock  (2026-08-06)

## Corpus Check
- 44 files · ~35,093 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 388 nodes · 598 edges · 18 communities (17 shown, 1 thin omitted)
...
1. `DockingApp` - 33 edges
2. `run_pipeline()` - 25 edges
3. `SettingsDialog` - 16 edges
4. `validate()` - 14 edges
...
Cohesion: 0.06
Nodes (23): Frame, build_chain_ligand_map(), parse_hetatm_records(), Path, Molecular Docking Pipeline ========================== hetatm_parser.py — HETATM…, Group candidate ligands by chain. Returns {chain_id: [list of candidate hetatm…, Read a PDB file and return one dict per unique HETATM residue group. Each dict…, LabelFrame (+15 more)

### Community 2 - "SettingsDialog"
Cohesion: 0.14
Nodes (14): Notebook, _initial_dir(), Tk, Molecular Docking Pipeline ===================================== settings.py —…, Modal Toplevel settings GUI dialog., Atomically write config.json., Persist config dict to config.json., Return a list of error strings for missing / invalid settings. An empty list… (+6 more)
...
- **Why does `run_pipeline()` connect `pipeline.py` to `DockingApp`, `ligand_prep.py`, `receptor_prep.py`?**
